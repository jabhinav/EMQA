...
class PeftMmModel(PushToHubMixin, torch.nn.Module):
	"""
...
class PeftMmModelForCausalLM(PeftMmModel):
	"""
	...
	def __init__(self, model, peft_config: PeftConfig, adapter_name="default"):
		...
		# Initialize the soft_v_prompt <- [Hack] will be used during generate API, not in FWD pass
		self.soft_v_prompt = None
	...
	def forward(
		...
		if peft_config.peft_type == PeftType.PREFIX_TUNING:
			...
			# [My Custom]
			if soft_v_prompt is None:
				raise ValueError("soft_v_prompt is required for Multi-modal-based prefix tuning")
			
			# # For Prefix
			inputs_embeds = torch.cat((soft_v_prompt, inputs_embeds), dim=1)
		...
		if peft_config.is_prompt_learning:
			...
			if model_kwargs["past_key_values"] is None and peft_config.peft_type == PeftType.PREFIX_TUNING:
				...
				if model_kwargs["past_key_values"] is None:
					...
					# # For Prefix
					model_kwargs["inputs_embeds"] = torch.cat((self.soft_v_prompt, inputs_embeds), dim=1)
