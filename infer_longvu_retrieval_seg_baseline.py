"""Baseline Details
- Each segment is represented by a single frame.
...
def load_clip_model(retrieval_model_path: str = "openai/clip-vit-large-patch14"):
	"""
...
def predict(args, annotations: dict) -> None:
	...
	ret_model, ret_processor = load_clip_model(args.retrieval_model_path)
	ret_model.eval()  # Set the model to evaluation mode
	...
	for video_id in annotations:
		...
		for i in range(0, len(frame_idxs), args.n):
			...
			segments.append(
				frame_idxs[start:end]
			)
		segment_idxs = [i for i in range(len(segments))]
		print(f"[INFO] {len(segment_idxs)} Segments Extracted Successfully")
		# Let's pick a representative frame from each segment
		if args.representative_strat == 'first':
			selected_frames = [segment[0] for segment in segments]
		elif args.representative_strat == 'middle':
			selected_frames = [segment[len(segment) // 2] for segment in segments]
		elif args.representative_strat == 'last':
			selected_frames = [segment[-1] for segment in segments]
		elif args.representative_strat == 'random':
			import random
			selected_frames = [random.choice(segment) for segment in segments]
		else:
			raise ValueError("Invalid strategy for selecting representative frames")
		# Compute embeddings for the representative frames as placeholders for segment embeddings
		segment_embeddings = []
		for frame_idx in tqdm(selected_frames, desc="Extracting Frame Embeddings", total=len(selected_frames)):
			...
			segment_embeddings.append(frame_embed)
		# [Output] -> So far we have segments, segment_idxs, representative frames, and embeddings
		...
		for qa_sample in annotations[video_id]:
			...
			# Sort the segment based on similarity scores and get top-k frame indices
			sorted_segment_scores = sorted(zip(segment_idxs, segments, selected_frames, similarity_scores), key=lambda x: x[-1], reverse=True)
			top_k_segments = [seg_idx for seg_idx, _, _, _ in sorted_segment_scores[:args.top_k]]
	...
	save_at = f"results_{model_name}_baseline_top{args.top_k}_segment_{args.representative_strat}RepFrame_{args.m}m_{args.n}n_retrieval.json"
...
if __name__ == "__main__":
	...
	parser.add_argument('--representative_strat', type=str, default="middle", choices=["first", "last", "middle", "random"])
